...
export function MarkdownPartial({
  ...
  function checkOverflow(node: HTMLDivElement) {
    ...
    /* "If the scrollcontainer size is larger than the client container size" */
  }

  useEffect(() => {
    /* "We use the useEffect to react to window size changes to adjust the max-length of the text"*/
  ...
  useLayoutEffect(() => {
    /*This Hooks runs when all content on the page is rendered*/
    if (markdownRef.current == null)
      return; /* Wenn die markdownRef null ist, mache nichts */
    checkOverflow(markdownRef.current);
  });
